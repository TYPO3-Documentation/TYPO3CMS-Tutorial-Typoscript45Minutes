...
- :ref:`CONTENT <t3tsref:cobj-content>` can be used to access arbitrary tables
  of TYPO3 CMS internals. This does not only include table "tt\_content", but
  extension tables can also be referenced. The :ref:`select <t3tsref:select>`
  function makes it possible to generate complex SQL queries.

- :ref:`RECORDS <t3tsref:cobj-records>` can be used to reference specific data
