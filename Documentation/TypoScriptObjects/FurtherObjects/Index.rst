...
- :ref:`CASE <t3tsref:cobj-case>` allows case differentiation. In the core this
  object is used for rendering different content elements according to their
  type.

- :ref:`COA <t3tsref:cobj-coa>` (content object array) allows us to combine an
  arbitrary number of objects.

- :ref:`COA\_INT <t3tsref:cobj-coa-int>` is a COA object, but non-cached. This
...
- :ref:`LOAD\_REGISTER <t3tsref:cobj-load-register>` /
  :ref:`RESTORE\_REGISTER <t3tsref:cobj-restore-register>` objects allow us to
  fill the global array :code:`$GLOBALS['TSFE']->register[]` with content.
...
- :ref:`USER <t3tsref:cobj-user>` and USER\_INT are for user-defined functions.
...
- :ref:`GIFBUILDER <t3tsref:gifbuilder>` is used for generating image files
