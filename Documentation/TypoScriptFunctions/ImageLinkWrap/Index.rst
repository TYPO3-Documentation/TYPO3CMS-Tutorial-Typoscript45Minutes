...
such as window background, image size, etc. This function can be used to
create a "click-to-enlarge" functionality. (A small image [thumbnail] is
clicked to open a new window and show the image in its original size.) ::
   ...
   temp.myImage {
           ...
           imageLinkWrap {
                   ...
                   # Activates ImageLinkWrap.
                   enable = 1

                   # Defines the body tag for the new window.
                   bodyTag = <body class="ImageOriginal">

                   # Wraps the image. (Closes the window, if it is clicked.)
                   ...
                   # Creates a new window for the image.
                   JSwindow = 1

                   # Opens a new window for every image (instead of using the same window).
